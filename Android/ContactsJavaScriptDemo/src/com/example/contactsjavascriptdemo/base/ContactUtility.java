...
public class ContactUtility {
    ...
    /**
     * Returns a JSON of the following form: { "contacts":[
     * {"key":"J","values":[ {"contactId":12,"displayName":"John Doe",
     * "key":"J"}, {"contactId":19,"displayName":"Jane Doe", "key":"J"} ] },
     * {"key":"K","values":[ {"contactId":12,"displayName":"Konur", "key":"K"},
    ...
    public static String getAllContactDisplaysJSON(ContentResolver contentResolver) {
        ...
        if (list == null || list.isEmpty()) { return EMPTY_CONTACT_LIST; }
    ...
    public static String getContactJSON(String id, ContentResolver contentResolver) {
        ...
        if (EMPTY.equals(id)) { return EMPTY_JSON; }
        Contact contact = getContact(id, contentResolver);
        return contact != null ? JSON.toJSONString(contact) : EMPTY_JSON;
    }
    ...
    private static Contact getContact(String id, ContentResolver contentResolver) {
        ...
        Cursor cursor = contentResolver.query(
                ...
                ContactsContract.Contacts._ID + QUESTION, new String[] { id },
                null);
        if (cursor.getCount() > 0) {
            contact = new Contact();
            while (cursor.moveToNext()) {
                ...
                String[] names = getFirstNameLastName(contactId,
                        contentResolver);
    ...
    public static void deleteContact(String id,
            ContentResolver contentResolver, String accountType) {
        HashMap<String, String> contacts = getUsersFromAccount(accountType,
                contentResolver);
    ...
    public static void saveOrUpdateContact(Contact contact,
            ContentResolver contentResolver, String accountName,
            String accountType) {
        if (contact == null || accountName == null || accountType == null) { return; }
    ...
    private static void deleteContactInternal(String id,
            ContentResolver contentResolver) {
        Cursor cursor = contentResolver.query(
                ContactsContract.Contacts.CONTENT_URI, null,
                ContactsContract.Contacts._ID + QUESTION, new String[] { id },
        ...
        if (cursor.getCount() > 0) {
            while (cursor.moveToNext()) {
                lookup = cursor.getString(cursor
                        .getColumnIndex(ContactsContract.Contacts.LOOKUP_KEY));
            }
        }
        cursor.close();
        Uri uri = Uri.withAppendedPath(
                ContactsContract.Contacts.CONTENT_LOOKUP_URI, lookup);
        contentResolver.delete(uri, null, null);
    }

    private static HashMap<String, String> getUsersFromAccount(
            String accountType, ContentResolver contentResolver) {
        Cursor cursor = contentResolver.query(
                ContactsContract.RawContacts.CONTENT_URI, null,
                ContactsContract.RawContacts.ACCOUNT_TYPE + QUESTION,
